...
john.shift(); // removes element from beginning of array
console.log(john);
...
To tip the waiter a fair amount, John created a simple tip calculator (as a
    function). He likes to tip 20% of the bill when the bill is less than
...
function tipCalculator(bill) {
    if (bill < 50) {
        tips.push(bill * 0.2)
        final.push((bill * 0.2) + bill)
    } else if (bill >= 50 && bill <= 200) {
        tips.push(bill * 0.15)
        final.push((bill * 0.15) + bill)
    } else {
        tips.push(bill * 0.10)
        final.push((bill * 0.10) + bill)
    }
}

tipCalculator(billsJohn[0]);
...
containing final paid amounts (bill + tip). HINT: Start with the two empty
...
array of tips. HINT: Looop over the array, and in each iteration store the
current sum in a variable (starting from 0). After you have the sum of the
array, divide it by the number of elements in it (that's how you calculate 
the average)
...
var john = {
    ...
    calcTip: function() {
        ...
        for (var i = 0; i < this.bill.length; i++) {
            ...
            this.finals.push((this.bill[i] * percentage) + this.bill[i]);
        }
...
var avgMark = calcAverage(mark.tips);

console.log(avgJohn, avgMark);

if (avgJohn > avgMark) {
    console.log('John\'s family paid the highest tips on average with $' + avgJohn);
} else if (avgMark > avgJohn) {
    console.log('Mark\'s family paid the highest tips on average with $' + avgMark);
} else {
    console.log('Both family paid the same amount on average.');
}
*/
